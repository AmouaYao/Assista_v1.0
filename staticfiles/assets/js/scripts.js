const WHATSAPP_NUMBER = "2250151177446";
      const CURRENCY = "FCFA";
      ...
      function updateCartBar() {
        const cnt = cartItemCount(),
          subtotal = cartTotal();
        const pourboireParArticle = 70;
        const pourboireTotal = cnt * pourboireParArticle;
      ...
      <div class="cart-item-right">
        ...
        const pourboireParArticle = 70;
        const pourboireTotal = itemCount * pourboireParArticle;
        const totalAvecPourboire = subtotal + pourboireTotal;
        ...
        if (!summaryHtml.includes('cartPourboire')) {
            summaryHtml += `
                <div class="summary-row" id="cartPourboire">
                    <span>TCA (${itemCount} article${itemCount > 1 ? 's' : ''})</span>
            ...
            if (pourboireElement) {
                ...
                pourboireLabel.textContent = `TCA (${itemCount} article${itemCount > 1 ? 's' : ''})`;
            }
            if (totalElement) {
      ...
      async function sendOrder() {
        ...
        const itemCount = cartItemCount();
        const pourboireParArticle = 70;
        const totalAvecPourboire = total + (itemCount * pourboireParArticle);

        // 1. Ouvrir d'abord le lien de paiement Wave
        const waveUrl = `https://pay.wave.com/m/M_ci_-5o8YqgPD6iB/c/ci/?amount=${totalAvecPourboire}`;
        ...
        try {
          ...
          if (result.success) {
            ...
            lines.push("TCA : " + result.pourboire.toLocaleString("fr-FR") + " FCFA");
            lines.push("Total à payer : " + result.total_avec_pourboire.toLocaleString("fr-FR") + " FCFA");
            lines.push("----------------------------------------");
            lines.push("Telephone client (envoi lien Wave) : " + phone);
            ...
            lines.push("----------------------------------------");
            lines.push("Merci de lui envoyer le lien de paiement Wave pour ce montant.");
      ...
      function wavePaymentUrl() {
        // Calculer le montant total avec TCA
        const total = cartTotal();
        const itemCount = cartItemCount();
        const pourboireParArticle = 70;
        const totalAvecPourboire = total + (itemCount * pourboireParArticle);
        
        // Créer l'URL Wave avec le montant
        const waveUrl = `https://pay.wave.com/m/M_ci_-5o8YqgPD6iB/c/ci/?amount=${totalAvecPourboire}`;
